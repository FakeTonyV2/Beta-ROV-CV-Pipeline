...
ERROR_CODE_CONTRACTS: dict[ErrorCode, ErrorCodeContract] = {
    ...
    ErrorCode.CAMERA_NOT_FOUND: _contract(
        "camera service",
        "valid removable camera is temporarily absent",
        "N/A (diagnostic)",
        "DEGRADED",
        "0",
        "restore the same stable identity; service retries",
        "test_camera_not_found",
        command_response_allowed=True,
    ),
    ErrorCode.CAMERA_MODE_UNSUPPORTED: _contract(
        "camera service",
        "requested exact V4L2 tuple is not advertised",
        "FAILED",
        "ERROR",
        "78",
        "correct deployment configuration and restart",
